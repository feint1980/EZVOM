...
namespace Feintgine {
	...
	Glyph::Glyph(const glm::vec4& destRect, const glm::vec4& uvRect, float depth, const Color& color, float angle, float spec, float lightPoint)
	{
		topLeft.color = color;
		topLeft.isLight = lightPoint;
		topLeft.multiplier = spec;
		topLeft.setPosition(destRect.x, destRect.y + destRect.w);
		topLeft.setUV(uvRect.x, uvRect.y + uvRect.w);

		bottomLeft.color = color;
		bottomLeft.isLight = lightPoint;
		bottomLeft.multiplier = spec;
		bottomLeft.setPosition(destRect.x, destRect.y);
		bottomLeft.setUV(uvRect.x, uvRect.y);

		bottomRight.color = color;
		bottomRight.isLight = lightPoint;
		bottomRight.multiplier = spec;
		bottomRight.setPosition(destRect.x + destRect.z, destRect.y);
		bottomRight.setUV(uvRect.x + uvRect.z, uvRect.y);

		topRight.color = color;
		topRight.isLight = lightPoint;
		topRight.multiplier = spec;
		topRight.setPosition(destRect.x + destRect.z, destRect.y + destRect.w);
		topRight.setUV(uvRect.x + uvRect.z, uvRect.y + uvRect.w);
	...
	void SpriteBatch::drawLight(const glm::vec4& destRect, const glm::vec4& uvRect,  float depth, const Color& color, float spec, float lightPoint)
	{
		_glyphs.emplace_back(destRect, uvRect, depth, color, 0,spec, 1);
	}
